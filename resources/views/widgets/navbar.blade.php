...
<nav class="navbar has-border-bottom is-dark" role="navigation" aria-label="main navigation">
    <div class="navbar-brand">
        <a class="navbar-item is-title-font" href="{{ url('/') }}"><img class="navbar-title-logo" src="{{ asset('logo.png') }}" alt="Logo"/>&nbsp;{{ env('APP_NAME') }}</a>
    ...
    <div id="navbarMenu" class="navbar-menu">
            ...
            <center>
                <div class="navbar-filter">
                    <div class="navbar-item has-dropdown is-hoverable is-inline-block">
                        <a class="navbar-link is-top-5" href="javascript:void(0);" onclick="window.vue.toggleDropdown(document.getElementById('type-dropdown'));">
                            {{ __('app.select_type') }}
                        </a>

                        <div class="navbar-dropdown is-dropdown-aligned is-hidden" id="type-dropdown">
                            @foreach (\App\Models\TypeModel::getTypes() as $item)
                                <a class="navbar-item" href="{{ url('/') }}?type={{ $item->slug }}">{{ $item->type }}</a>
                            @endforeach
                        </div>
                    </div>
        ...
        <div class="navbar-end">
            ...
            <div class="navbar-item">
                <div>
                    <i class="fas fa-upload fa-lg is-pointer is-color-nav-bright" title="{{ __('app.submit_item') }}" onclick="location.href='{{ url('/submit') }}';"></i>&nbsp;<span class="is-mobile-like-screen-width"><a class="is-color-grey" href="javascript:void(0);" onclick="location.href='{{ url('/submit') }}';">{{ __('app.submit_item') }}</a></span>
                </div>
            </div>
