...
class AdminController extends Controller
{
    ...
    {
        $approvals = ItemModel::where('approved', '=', false)->orderBy('id', 'asc')->limit(env('APP_APPROVALFETCHCOUNT'))->get();
        ...
        $reports = array(
            'items' => ReportModel::getReportPack('ENT_ITEM'),
    ...
    public function approveItem($id)
    {
        try {
            ...
            PushModel::addNotification(__('app.item_approved_short'), __('app.item_approved_long', ['name' => $item->name, 'url' => url('/view/' . $item->slug)]), 'PUSH_APPROVAL', $item->userId);
            ...
            return back()->with('flash.success', __('app.item_approved'));
        } catch (\Exception $e) {
            return back()->with('error', $e->getMessage());
        }
    }
    ...
    public function declineItem($id)
    {
        try {
            ...
            return back()->with('flash.success', __('app.item_declined'));
        } catch (\Exception $e) {
            return back()->with('error', $e->getMessage());
        }
    }
